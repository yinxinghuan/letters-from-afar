...
const directory = await mkdtemp(join(tmpdir(), 'lettersFromAfar-story-session-'))
...
try {
  ...
  const store = new Store(); const scope = 'lettersFromAfar:deployment:owner-a:zh'
  ...
  const foreign = await client('token-b').enroll(createInitialSave(lettersFromAfar), 'owner-b-enrollment')
  ...
  const rollbackSession = await client().enroll(createInitialSave(lettersFromAfar), 'rollback-enrollment')
