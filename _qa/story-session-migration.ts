...
const directory = await mkdtemp(join(tmpdir(), 'lettersFromAfar-migration-'))
...
const body = (version: number, ruleset = 1, id = LETTERS_FROM_AFAR_SAVE_REPAIR_MIGRATION_ID) => JSON.stringify({ migration_id: id, expected_version: version, ruleset_version: ruleset })
...
try {
  const seed = createStorySessionLab({ cartridge: lettersFromAfar, databasePath, actorTokens: { 'token-a': 'owner-a', 'token-b': 'owner-b' }, generator: { async send(): Promise<never> { throw new Error('unused') } } })
  const seedUrl = (await seed.listen()).baseUrl; const enrolled = await clientFor(seedUrl).enroll(createInitialSave(lettersFromAfar), 'migration-seed'); await seed.close()
  ...
  const make = () => createStorySessionLab({ cartridge: lettersFromAfar, databasePath, actorTokens: { 'token-a': 'owner-a', 'token-b': 'owner-b' }, beforeMigrationCommit: async () => { waiting += 1; if (waiting === 2) release(); await barrier }, generator: { async send() { return { content: '旧邮局的信封、盐痕与路线记录被重新核对。\n[scene_location: location="漂港·旧邮局"]\n[choices: "保存盐痕记录"|"核对盐沼旧堤路线"]' } } } })
  ...
  const extra = await fetch(`${oneUrl}/api/story/sessions/${enrolled.session_id}/migrations`, { method: 'POST', headers: headers(), body: JSON.stringify({ migration_id: LETTERS_FROM_AFAR_SAVE_REPAIR_MIGRATION_ID, expected_version: 0, ruleset_version: 1, snapshot: {} }) })
  assert.equal(extra.status, 400); assert.equal(auditCount(), 0); assert.equal(readHead(enrolled.session_id), before)

  const results = await Promise.all([c1.migrate(enrolled.session_id, LETTERS_FROM_AFAR_SAVE_REPAIR_MIGRATION_ID, 0), c2.migrate(enrolled.session_id, LETTERS_FROM_AFAR_SAVE_REPAIR_MIGRATION_ID, 0)])
  ...
  const reopened = createStorySessionLab({ cartridge: lettersFromAfar, databasePath, actorTokens: { 'token-a': 'owner-a' }, generator: { async send(): Promise<never> { throw new Error('unused') } } })
  const reopenedUrl = (await reopened.listen()).baseUrl; const replay = await clientFor(reopenedUrl).migrate(enrolled.session_id, LETTERS_FROM_AFAR_SAVE_REPAIR_MIGRATION_ID, 0)
  ...
  const rollback = createStorySessionLab({ cartridge: lettersFromAfar, databasePath: rollbackPath, actorTokens: { rollback: 'owner' }, failMigrationBeforeCommit: [LETTERS_FROM_AFAR_SAVE_REPAIR_MIGRATION_ID], generator: { async send(): Promise<never> { throw new Error('unused') } } })
  const rollbackUrl = (await rollback.listen()).baseUrl; const rollbackClient = clientFor(rollbackUrl, 'rollback'); const rollbackSession = await rollbackClient.enroll(createInitialSave(lettersFromAfar), 'rollback')
  await assert.rejects(rollbackClient.migrate(rollbackSession.session_id, LETTERS_FROM_AFAR_SAVE_REPAIR_MIGRATION_ID, 0), { code: 'INTERNAL_ERROR' })
  assert.equal((await rollbackClient.read(rollbackSession.session_id)).version, 0)
  assert.equal((await rollbackClient.migrate(rollbackSession.session_id, LETTERS_FROM_AFAR_SAVE_REPAIR_MIGRATION_ID, 0)).version, 1)
