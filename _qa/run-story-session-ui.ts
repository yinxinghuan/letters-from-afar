...
const gameBase = '/65b050fc-97b2-4513-b95a-a8dc25419896'
...
for (const locale of ['zh','en'] as const) {
  const service = createStorySessionLab({ cartridge: resolveCartridge(null, locale), databasePath: resolve(directory, `${locale}.sqlite`), actorTokens: { 'qa-ui-a':'qa-ui-a','qa-ui-b':'qa-ui-b' }, generator: { async send(_action, context) { modelCalls += 1; return { content: locale === 'zh' ? `你拆开那封受潮的信，封口夹着粗盐，字里行间指向旧堤道的北侧邮亭。\n[state: value="核对粗盐与旧堤道邮亭的来信"]\n[scene_location: location="${context.save.sceneLocation ?? context.save.location}"]\n[choices: "沿信纸水痕判断来路"|"询问邮亭最近经过的信使"]` : `You open the damp letter. Coarse salt clings to the seal, and the writing points toward the post shelter north of the old causeway.\n[state: value="Trace the salted letter to the causeway post shelter"]\n[scene_location: location="${context.save.sceneLocation ?? context.save.location}"]\n[choices: "Read the water marks for its route"|"Ask which courier passed the shelter"]` } } } })
  services.set(locale, service); bases.set(locale, (await service.listen()).baseUrl)
}
const vite = await createServer({ configFile: resolve('vite.config.ts'), server: { host:'127.0.0.1', port:Number(process.env.STORY_LAB_UI_PORT ?? 5190), strictPort:true }, plugins:[{ name:'letters-from-afar-story-session-qa-only', configureServer(server) { server.middlewares.use(async (request,response,next) => {
