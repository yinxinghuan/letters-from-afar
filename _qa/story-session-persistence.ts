...
const directory = await mkdtemp(join(tmpdir(), 'lettersFromAfar-session-process-'))
...
try {
  const first = await start(); const second = await start(); const initial = createInitialSave(lettersFromAfar); const opening = initial.choices[0]!.label
  ...
  const contractOptions = { cartridge: lettersFromAfar, databasePath, actorTokens: {}, generator: { async send(): Promise<never> { throw new Error('unused') } } }
  assert.throws(() => createStorySessionLab({ ...contractOptions, rulesetVersion: 2 }), /DATABASE_CONTRACT_MISMATCH/)
  assert.throws(() => createStorySessionLab({ ...contractOptions, cartridge: { ...lettersFromAfar, id: 'different-game' } }), /DATABASE_CONTRACT_MISMATCH/)
