...
try {
  ...
  const privateSave = createInitialSave(lettersFromAfar); privateSave.objective = 'PRIVATE_OWNER_A_SENTINEL'
  const first = await a.enroll(privateSave, 'owner-a-first')
  const second = await a.enroll(createInitialSave(lettersFromAfar), 'owner-a-second')
  const foreign = await b.enroll(createInitialSave(lettersFromAfar), 'owner-b-only')
  ...
  const opened = await journal.open(createInitialSave(lettersFromAfar))
  ...
  const restarted = await journal.restart(createInitialSave(lettersFromAfar))
  ...
  enrollmentStore.raw = JSON.stringify({ schema: 1, scope, enrollment: { id: 'new-session', initialSave: createInitialSave(lettersFromAfar) } })
