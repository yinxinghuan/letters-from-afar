...
assert.ok(lettersFromAfar.audioTheme.levels.sfx <= .045, 'reading-mode synthesized feedback stays quiet')
assert.ok((lettersFromAfar.audioTheme.recorded?.cues?.travel?.gain ?? 1) <= .12, 'route-arrival recording stays below the reading foreground')
...
let rail = replay(lettersFromAfar, ['邮戳', '档案柜', '盐沼旧堤', '投信箱', '核对最近三次退潮', '旧公路旅舍', '莉娜'])
...
let plateau = replay(lettersFromAfar, ['艾达', '档案柜', '北渡口', '稳住船板', '辨认能沾上蓝灰树脂', '杉湖镇', '未署名的旧信'])
...
let railEn = replay(lettersFromAfarEn, ['postmark', 'archive cabinet', 'Saltmarsh', 'letter box', 'last three low tides', 'Old Highway Lodge', 'Lena'])
...
let plateauEn = replay(lettersFromAfarEn, ['postmark', 'archive cabinet', 'North Ferry', 'hull plank', 'blue-grey resin', 'Cedar Lake', 'unsigned old letters'])
