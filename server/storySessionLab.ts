...
export const LETTERS_FROM_AFAR_SAVE_REPAIR_MIGRATION_ID = 'letters-from-afar-save-v10-repair-2026-09-04'
...
function sharedAuthorityProjection(save: StorySave) {
  return {
    items: save.inventory.filter(item => item.id.startsWith('shared-relay:')).map(item => ({ id: item.id, count: item.count })).sort((left, right) => left.id.localeCompare(right.id)),
    receipts: save.blocks.filter(block => block.id.startsWith('relay-receipt-') || typeof block.data?.relayReceiptId === 'string').map(block => ({ id: block.id, relayReceiptId: block.data?.relayReceiptId, relayId: block.data?.relayId, operation: block.data?.operation })).sort((left, right) => left.id.localeCompare(right.id)),
  }
}
function hasSharedAuthorityMarkers(save: StorySave) { const projection = sharedAuthorityProjection(save); return projection.items.length > 0 || projection.receipts.length > 0 }
function sharedAuthorityUnchanged(before: StorySave, after: StorySave) { return fingerprint(sharedAuthorityProjection(before)) === fingerprint(sharedAuthorityProjection(after)) }
...
export function createStorySessionLab(options: {
  ...
  validateSharedEnrollment?: (save: StorySave, owner: string) => boolean
}) {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/api/story/sessions') {
        ...
        if (hasSharedAuthorityMarkers(imported) && !options.validateSharedEnrollment?.(imported, owner)) throw new RequestError(409, 'SHARED_AUTHORITY_REQUIRED')
      ...
      if (migrationMatch) {
        ...
        if (Object.keys(body).some(key => !['migration_id','expected_version','ruleset_version'].includes(key)) || body.migration_id !== LETTERS_FROM_AFAR_SAVE_REPAIR_MIGRATION_ID || !counter(body.expected_version) || !counter(body.ruleset_version)) throw new RequestError(400, 'INVALID_MIGRATION')
        ...
        if (!sharedAuthorityUnchanged(current.snapshot, migrated)) throw new RequestError(409, 'SHARED_AUTHORITY_REQUIRED')
      ...
      if (!sharedAuthorityUnchanged(current.snapshot, executed.save)) { send(response, 409, { code: 'SHARED_AUTHORITY_REQUIRED' }); return }
