...
import { buildDangerDirective, createDangerFallbackScene, normalizeDangerState, repairLegacyDangerLoopChoices, repairLegacyDangerMethodChoices, resolveActiveDangerDeflection } from './engine/dangerDirector'
...
import type { RelayReceipt } from '../shared-world/types'
import { applyRelayReceiptToSave } from '../shared-world/receipt'
...
export function normalizeSave(candidate: LegacyStorySave | null | undefined, cartridge: StoryCartridge, incomingChatId?: string): StorySave {
  ...
  } as StorySave, cartridge)), cartridge)
...
export function useStoryEngine(cartridge: StoryCartridge, initialMode: StoryMode, incomingChatId?: string, imageIdentity: { ready: boolean; refUrl?: string } = { ready: true }) {
  const cloud = useGameSave<PersistedStoryData>('letters-from-afar')
  ...
  const applyRelayReceipt = useCallback((receipt: RelayReceipt) => new Promise<void>((resolve) => {
    commit((current) => applyRelayReceiptToSave(current, receipt, cartridge.locale))
  ...
  return { save, mode, setMode, busy, progress, error, pendingAction, canRetry: Boolean(failedAction), enter, act, retryAction, useAigramFallback, retryImage, prepareInventoryImages, applyRelayReceipt, restartWorld, loaded: cloud.loaded && seeded.current, clear: cloud.clear }
}
